...
# Ensure output and output/figures/ directories exist                                                                                                       
@default_files = ('index.tex');

# General settings
$out_dir = 'output';
$aux_dir = 'output';
foreach my $dir ($out_dir, "$aux_dir/figures") {                                                                                                            
    if ( ! -e $dir ) {                                                                                                                                    
...
$make_fmt = '%C %O --shell-escape -ini -output-directory=%V -jobname=%B "&%C" mylatexformat.ltx %S';
...
foreach my $engine ( 'latex', 'pdflatex' ) {
    ...
    ${$engine} = "internal mylatex %R $engine %O -interaction=nonstopmode -synctex=1 --shell-escape  %S";
...
sub make_fmt_cus_dep {
   ...
   # Implementation of "STILL TO DO 2":
   # Build format with jobname = $root to avoid TikZ externalization issues.
   # We must protect the main logs/fls from being clobbered.
   my $r_log = "$path$root.log";
   my $r_fls = "$path$root.fls";
   my $r_log_bak = "$path$root.log.bak";
   my $r_fls_bak = "$path$root.fls.bak";
   
   rename($r_log, $r_log_bak) if -e $r_log;
   rename($r_fls, $r_fls_bak) if -e $r_fls;
   ...
   # Use $root for %B in Run_subst
   my $ret = Run_subst( $cmd, 2, $options, $source, $dest, $root );

   # Runsubst with jobname=$root will have produced $path$root.fmt.
   # But latexmk expects $dest (e.g. $path$base.fmt).
   if (-e "$path$root.fmt" && "$path$root.fmt" ne $dest) {
       rename("$path$root.fmt", $dest);
   }

   # Extract dependencies from the format build's fls
   my @source_files = get_fls_inputs( $r_fls );
   
   # Clean up and restore
   unlink($r_log) if -e $r_log;
   unlink($r_fls) if -e $r_fls;
   rename($r_log_bak, $r_log) if -e $r_log_bak;
   rename($r_fls_bak, $r_fls) if -e $r_fls_bak;
