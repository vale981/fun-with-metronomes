...
\begin{align}
  ...
  \label{eq:eom_oscillator}\ddot{x}_{n}+\ddot{x}_{t} &= -\frac{k}{m} x_{n} +\color{gray}{\mu \pqty{\hat{x}^{2}-x_{n}^{2}}\dot{x}_{n}},
\end{align}
...
\begin{align}
  ...
  \label{eq:eom_fourier_modes}\ddot{y}_{k}+\delta_{k,0}\ddot{x}_{t} &= -\frac{k}{m} y_{k} +{\frac{\mu}{m} \pqty{\hat{x}^{2}\dot{y}_{k}-\sum_{pq}y_{p}y_{q}\dot{y}_{k-(p+q)}}}.
\end{align}
...
\begin{equation}
  ...
  \frac{\hat{x}^{2}\mu^{2}-km_{k}}{6m^{2}_{k}}t^{3} \dot{y}_{k}(0),
\end{equation}
where \(m_{0}=m_{\text{eff}}\) and \(m_{k\neq 0}=m\). Therefore, we can expect that for \(\hat{x}^{2}\mu^{2} \gg k m_{\text{eff}}\) the \(k=0\) mode remains strongly favored. Conversely, if \(\mu\) or \(\hat{x}\) is too small, we can expect the \(k=0\) mode to compete with the other modes.
...
\begin{equation}
  ...
  \tilde{\mu} =\frac{\mu \hat{x}^{2}}{m_{\text{eff}} \omega_{0}} = \frac{\mu\hat{x}^{2}}{\sqrt{km_{\text{eff}}}}.
\end{equation}
...
\begin{equation}
  ...
  \delta\ddot{y}
  \approx-\frac{k}{m} \delta y +\mu \bqty{\hat{x}^{2} - \Bqty{3+(N-3)N} (\delta y)^{2} - \ev{y_{0}^{2}} }\delta\dot{y}
\end{equation}
